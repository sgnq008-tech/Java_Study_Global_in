...
/* 문]
 *      주민번호를 입력받아 올바른 주민번호인지를 검증하는 프로그램을 구현하시오.
 *      
 *      123456 - abcdefg;
 *      12 -> 년도, 34 - > 월, 56 -> 일
 *      
 ...
 *         hap += 주민번호의 여섯번쨰 자리 * 7
 *          - : 계산하지 않음
 *         hap += 주민번호의 여덟번쨰 자리(a) * 8
 *         hap += 주민번호의 아홉번쨰 자리(b) * 9
 *         hap += 주민번호의 열번쨰 자리(c) * 2
 *         hap += 주민번호의 열한번쨰 자리(d) * 3
 *         hap += 주민번호의 열두번쨰 자리(e) * 4
 *         hap += 주민번호의 열세버쨰 자리(f) * 5
 *         
 *         float temp = (int)(hap / 11.0f) * 11.0f + 11.0f -hap;
 *         float temp1 = temp - (int)(temp / 10.0f) * 10.0f;
 *         주민번호가 옳은지 틀린지 알 수 있다.
 * 
 */
import java.util.*;
import java.io.*;
	...
	public static void main(String[] args) throws IOException{
		...
		do {
			...
			if(jumin[2]*10 + jumin[3] > 12) {
				System.out.println("태어난 날은 12월보다 클 수 없습니다.");
				continue;
				...
				System.out.print("성별을 나타내는 숫자는 9,0,1,2,3,4 중 하나만 가능합니다.");
				continue;
			}
